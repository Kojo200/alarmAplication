...
function handleAlarmManagerMouseMoveOnScreen(
  mouseX,
  mouseY,
  canvasWidth,
  canvasHeight,
...
function handleAlarmManagerClickOnScreen(
  mouseX,
  mouseY,
  canvasWidth,
  canvasHeight,
  ...
  if (
    mouseX >= canvasWidth - 60 &&
    mouseX <= canvasWidth - 20 &&
    mouseY >= startY + 5 &&
    mouseY <= startY + 45
  ) {
    createAlarm(8, 0);
  ...
  for (let i = 0; i < alarms.length; i++) {
    ...
    if (getClockFormat() === "12h") {
      ...
      if (alarm.hours === 0) {
        ...
        displayHours = alarm.hours;
        period = "AM";
      } else if (alarm.hours === 12) {
        displayHours = 12;
        period = "PM";
      } else {
        displayHours = alarm.hours - 12;
        period = "PM";
      }
      ...
      timeDisplay =
        String(alarm.hours).padStart(2, "0") +
        ":" +
        String(alarm.minutes).padStart(2, "0");
    }
